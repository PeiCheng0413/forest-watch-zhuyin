...
| 滑鼠點擊法術 | 啟用／停用法術，至少保留一種 |
| Tab 或 ← / → | 只在啟用的法術間切換，保留目前輸入；Tab 只正向循環 |
...
| 法術 | 目前效果 | 用途 |
| --- | --- | --- |
| 中毒 | 80 半徑範圍；持續 10 秒，每秒 6 點傷害；每秒傳染附近一隻未中毒魔物 | 削弱密集怪群 |
| 石化 | 120 半徑範圍，停止移動與攻擊 4.5 秒 | 阻擋逼近或正在攻塔的敵人 |
| 燃燒 | 持續 3.5 秒，每秒 13 點傷害 | 快速處理危險敵人 |
| 混亂 | 持續 6 秒，移速提高至 1.8 倍，接近並攻擊其他魔物 | 利用怪群互相傷害 |

法術沒有冷卻時間。同種效果再次施放會刷新時間，傷害不疊倍；不同效果可以並存。石化期間中毒與燃燒仍會造成傷害；混亂的怪物若沒有其他目標，暫時停止行動。

## 魔石、大魔法與難度

每隻魔物消散後自動獲得一顆魔石，魔石飛向右下角魔杖，周圍的光由下往上累積。第一輪集滿 **12 顆**後魔杖發光抖動，戰鬥與計時繼續，能量保留至施放。輸入列空白時，在 **450 毫秒**內連按兩下空白鍵，才進入咒語模式，完全暫停戰鬥與存活計時。長按不算連按；中途輸入其他按鍵、切換法術、暫停或離開視窗會取消連按判定。輸入名字中的空白鍵仍是一聲「ˉ」，不會觸發大魔法。正確輸入注音咒語並按 Enter 後清場，能量歸零，下次需求增加 1 顆（13、14、15……），重新開始恢復 12 顆；大魔法清場不回補魔石，避免連續觸發。
...
| `dist/assets/forest-staff.png` | 像素風木製法杖，同時作為蓄光與施法發光的輪廓遮罩 |
...
node tests/spells.cjs
node tests/poison.cjs
node tests/blessing.cjs
node tests/area.cjs
node tests/ex.cjs
...
- 以透明像素風木製法杖替換原本 SVG 圖案：深褐木紋、舊金色纏箍、紫色魔石。
- 移除球形蓄光，改用法杖貼圖的透明輪廓遮罩，由下往上沿杖身亮起；滿能量時全杖金光與抖動。
- 發光層放在貼圖後方，保留木頭與金屬材質；施法時貼圖與光層一起揮動，各法術顏色及大魔法金光保留。
- 保留右下角位置、魔石數字、一聲空白鍵與雙空白吟唱操作。
- 已用瀏覽器檢查半滿與蓄滿畫面，並驗證透明背景及原有操作測試。

### v1.8.0 自訂法術循環

- 滑鼠點擊法術按鈕切換啟用／停用，停用按鈕變暗並標示「已停用」；目前法術標示「使用中」。
- Tab 與左右鍵略過停用法術。關閉目前法術會自動切到下一個啟用法術，保留注音輸入；最後一種不能停用。
- 啟用選擇儲存於此瀏覽器的 `forest-watch-spells-v1`，重新開始與下次開啟會沿用；無效或全關閉的儲存資料會回復預設全開。儲存受限時仍可當場使用。
- Tab 只切換，輸入真名後仍按 Enter 施法。Shift + Tab 不作反向法術切換。
- 單元測試與瀏覽器實測涵蓋只保留兩種法術、切換保留輸入、至少一種、重開與重新載入保留設定。

### v1.9.0 中毒傳染

- 中毒每秒向 80 個戰場座標單位內最近的一隻未中毒魔物傳播，約兩隻哥布林身寬。
- 傳染持續時間取 5 秒與來源剩餘時間的較小值；新感染者需再等一秒才可接力。傳染不刷新已中毒魔物，玩家重新施法仍可刷新為 10 秒。
- 傳染呈現綠色孢子與「中毒傳染」提示，不增加施法次數或輸入嘗試。石化不阻止毒素傳播；死亡、暫停及吟唱期間不繼續傳播。
- 中毒仍為每秒 6 點傷害，燃燒維持原本設定。減少動態效果時以短暫連線取代移動孢子。
- 回歸測試涵蓋範圍、頻率、接力延遲、效果到期、死亡、凍結及手動刷新。

### v1.10.0 弓箭手永久祝福

- 每次成功完成大魔法，本場祝福加一層，每輪箭數為 1 加祝福層數，不設遊戲上限。本場不會到期，重新開始歸零。
- 射擊間隔維持 1.45 秒，每箭 9 點傷害。每輪先按距離排序存活魔物，優先分配給不同魔物，剩餘箭數輪流分配；只有一隻時承受全部箭數。
- 以同時齊射的目標快照分配，傷害按箭數整批計算，不隨箭數逐支迴圈。每輪最多呈現 12 支飛行特效，實際傷害不減少。
- 弓箭手腳下新增金色光環，箭塔旁顯示祝福層數及每輪箭數；同目標的多箭使用分離的出發位置。
- 驗證基本箭數、雙箭、多目標輪分、大量箭數、祝福疊加、暫停保留及重新開始重設。

### v1.11.0 範圍控制與混亂加速

- 中毒以每隻真名吻合魔物為中心，套用 80 戰場座標單位半徑的範圍效果，完整持續 10 秒並保留傳染。
- 石化採較大的 120 半徑，完整持續 4.5 秒。施法範圍以落地光圈呈現，提示實際命中數量。
- 同名多中心以受影響魔物集合去重，同一隻每次只套用一次，仍算一次成功施法；燃燒與混亂沒有新增範圍。
- 混亂期間移速為 1.8 倍，原始速度、傷害及攻擊間隔不變。石化優先停止移動，無其他魔物時停留；移動限制於攻擊距離，避免加速越過目標。
- 已驗證範圍邊界、重疊去重、非範圍法術、移速倍率、石化與無目標狀態，並以瀏覽器檢查範圍提示。

### v1.12.0 一次性 EX 獎勵與隕石

- 完成大魔法後，四種法術各補滿一次 EX，未使用次數不疊加；成功輸入並施法才消耗該種 EX，同名多目標算一次。錯字、停用與切換不消耗。重開歸零，法術啟用設定保留。
- 中毒 EX 半徑 200、石化 EX 半徑 300，均為普通版的 2.5 倍，持續時間不變。
- 燃燒 EX 改名「隕石」：在每隻同名魔物施法時的位置標記半徑 300，0.6 秒後對落地當下範圍內的魔物造成 60 點傷害。同次施法範圍重疊只計一次，不附加燃燒。
- 隕石呈現落點預告、帶火尾的岩塊墜落、爆炸波與短暫震動；減少動態效果時保留落點提示，取消墜落移動與震動。暫停與吟唱凍結待落地隕石，重新開始或大魔法清場取消未落地隕石。
- 混亂 EX 先影響同名魔物，再從其餘未混亂的存活魔物中隨機選取三分之一（無條件進位）施加混亂，保留 1.8 倍移速。
- 按鈕標示 EX 就緒，使用後恢復普通名稱；下次大魔法再次補充。弓箭手永久祝福照常疊加。
- 已驗證次數獨立消耗與補充、隕石重疊去重與移動後命中、暫停、魔石結算、混亂抽取及普通法術恢復。

### v1.13.0 像素火焰與隕石細節

- 燃燒採用暗橘、橘金與亮色焰心的分層像素火舌，搭配少量上升火星與淡煙，保持注音名字清晰。
- 燃燒魔物加入貼合角色輪廓的暖色反光；隕石加入岩塊明暗、熔岩裂紋、漸細尾焰與落地餘燼。
- 特效隨戰鬥時間播放，暫停與吟唱時凍結；減少動態效果時降低動態粒子與位移。
- 本次僅調整視覺，傷害、範圍、持續時間及 EX 規則維持不變。已完成語法、EX、施法回饋與角色視覺測試，並以瀏覽器確認燃燒與隕石施放。

### v1.13.1 載入修復嘗試（已由 v1.14.0 取代）

- 此版曾讓圖片失敗或等待 8 秒後放行；審查發現並沒有備援角色，因此此做法已移除。

### v1.15.0 大魔法需求逐次增加

- 每場初始需求為 12 顆魔石；每次成功完成大魔法後，下次需求加 1（13、14、15……），不設遊戲上限。能量歸零，重新開始恢復 12 顆。
- 進入吟唱或輸入錯誤不增加需求。法杖數字、蓄光比例與蓄滿判斷均依目前需求更新，弓箭手祝福與 EX 補充保留。
- 已驗證多次加價、差一顆不可啟動、錯字不加價、新能量上限及重新開始。

### v1.14.1 慢速載入自動恢復

- 等待 15 秒僅標記「載入較慢」，保留載入完成與錯誤監聽；圖片稍後完成會自動更新進度，全部完成後可開始，不需重新整理。
- 真正收到圖片錯誤事件才標記失敗，慢速與失敗名稱分開顯示。手動重試僅處理未完成素材，並清除上一輪監聽與計時器。
- 回歸測試涵蓋逾時後完成、失敗重試、慢速重試與監聽不重複。全部素材成功才開放開始的規則保留。

### v1.14.0 完整素材載入與法術光影

- 五項素材（森林、弓箭手、魔物、法杖、故事卷軸）全部載入成功才開放開始，不使用備援素材。進度依成功項目數呈現，並非下載位元組比例。
- 載入失敗或等待 15 秒後列出素材名稱與重試按鈕；重試只下載未完成項目，保留成功項目。序章與開始畫面均可看到進度。
- 新增綠色中毒、灰藍石化、橘色燃燒、紫色混亂、隕石與大魔法的地面光暈和魔物輪廓內反光。地面採分層多邊形光圈，保留像素風格。
- 最多同時繪製 32 個光源，角色反光強度限制為 0.28；減少動態效果時停止光源脈動。注音名稱最後繪製，不受光效染色。
- 新增快取成功／失敗、逾時、部分失敗與重試測試；以瀏覽器驗證失敗時禁止開始、重試恢復、四種光影及隕石命中後持續運作。
