...
document.querySelectorAll('[data-spell]').forEach(b=>b.onclick=()=>{if(game.mode==='playing'){game.clearSpaceTap();game.spell=+b.dataset.spell;renderUI()}});
...
function pulseWand(color,ultimate=false){
 ...
 burst.animate(reducedMotion.matches?[{opacity:1},{opacity:0}]:[{opacity:1,transform:'scale(.3)'},{opacity:.9,offset:.25},{opacity:0,transform:`scale(${ultimate?2.6:1.7})`}],{duration:ultimate?1100:650});
 if(!reducedMotion.matches){const art=document.querySelector('.wand-art');for(const a of art.getAnimations())a.cancel();art.animate([{transform:'rotate(0deg)'},{transform:'rotate(-13deg)',offset:.25},{transform:'rotate(4deg)',offset:.6},{transform:'rotate(0deg)'}],{duration:450})}
}
