(function(root){
 ...
 class AssetLoader {
  ...
  snapshot(){const done=this.items.filter(i=>i.status==='ready').length;return {done,total:this.items.length,ready:done===this.items.length,failed:this.items.filter(i=>i.status==='failed').map(i=>i.name)};}
  load(retry=false){for(const item of this.items){if(item.status==='ready'||item.status==='loading')continue;this.loadOne(item,retry);}this.changed(this.snapshot());}
  loadOne(item,retry){
   item.status='loading';const attempt=++item.attempt,img=item.image;
   const finish=ok=>{if(attempt!==item.attempt)return;this.clearTimer(item.timer);img.removeEventListener('load',loaded);img.removeEventListener('error',failed);item.status=ok?'ready':'failed';this.changed(this.snapshot());};
   ...
   item.timer=this.setTimer(failed,this.timeout);
