...
const {g}=setup(2,0);for(let i=1;i<=3;i++){g.energy=g.energyMax;g.input='';g.activateUltimate();g.input=g.chant.answer;g.submit();assert.equal(g.blessings,i);assert.equal(g.energy,0)}
