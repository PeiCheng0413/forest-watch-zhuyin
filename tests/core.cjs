...
g.input='su';g.spawn();g.select(1);assert.equal(g.input,'');g.input='s';g.changeSpell(1);assert.equal(g.input,'s');g.type('Backspace');assert.equal(g.input,'');
...
const sim=new Game(()=>.7);sim.reset();let ticks=0;while(sim.mode!=='ended'&&ticks++<24000){if(sim.mode==='ritual'){sim.input=sim.chant.answer;sim.submit()}sim.update(.05)}assert.equal(sim.mode,'ended');console.log('PASS: 注音鍵位／詞庫、錯字清空、四法術並存、切怪清空、刪字、暫停、儀式凍結與清場、石化防傷、怪物攻塔、死亡結算。');console.log('無操作生存時間：',Math.round(sim.time),'秒');
