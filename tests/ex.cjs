...
let {g,es}=setup();g.energy=g.energyMax;g.activateUltimate();g.input=g.chant.answer;g.submit();assert.deepEqual(g.exReady,[true,true,true,true]);g.toggleSpell(1);assert.ok(g.exReady[1]);g.input='invalid';g.submit();assert.ok(g.exReady.every(Boolean));g.energy=g.energyMax;g.activateUltimate();g.input=g.chant.answer;g.submit();assert.equal(g.blessings,2);assert.deepEqual(g.exReady,[true,true,true,true]);g.reset();assert.ok(g.exReady.every(v=>!v));
