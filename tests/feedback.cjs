...
for(let spell=0;spell<4;spell++){
 g.targetId=e.id;g.spell=spell;g.input=e.entry.answer;g.events=[];g.submit();let cast=g.events.find(e=>e.type==='cast');assert.equal(cast.targetId,e.id);assert.equal(cast.targetLabel,'ㄕㄢˉ');assert.equal(cast.spell,spell);assert.equal(cast.refreshed,false);
 g.targetId=e.id;g.input=e.entry.answer;g.events=[];g.submit();assert.equal(g.events.find(e=>e.type==='cast').refreshed,true);
}
