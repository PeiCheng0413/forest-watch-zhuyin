...
game.input='g';assert.equal(press('Tab').prevented,false);assert.equal(game.input,'g');press('Tab',{shiftKey:true});assert.equal(game.input,'g');game.input='';
...
console.log('PASS: 實際鍵盤處理器左右切法術、組字旗標、Tab 保留原生焦點操作、長按與一聲空白鍵。');
